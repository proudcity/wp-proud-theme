...
var tzlibZonesDB = {
  ...
  Chile: {
    ...
    EasterIsland: ['Pacific/Easter', 81]
  },
  CST6CDT: ['', 20],
...
function tzlib_get_content(tzName) {
  ...
  if (nameParts.length === 3) {
    return [tzlibZonesDB["".concat(nameParts[0])]["".concat(nameParts[1])]["".concat(nameParts[2])][0], tzlibZonesDetailsDB[tzlibZonesDB["".concat(nameParts[0])]["".concat(nameParts[1])]["".concat(nameParts[2])][1]]];
  }

  if (nameParts.length === 2) {
    return [tzlibZonesDB["".concat(nameParts[0])]["".concat(nameParts[1])][0], tzlibZonesDetailsDB[tzlibZonesDB["".concat(nameParts[0])]["".concat(nameParts[1])][1]]];
  }

  return [tzlibZonesDB["".concat(nameParts[0])][0], tzlibZonesDetailsDB[tzlibZonesDB["".concat(nameParts[0])][1]]];
...
function tzlib_get_offset(tzName, isoDate, isoTime) {
  ...
  var dateHour = date.getHours(); // preparing the tz data

  var timezoneData = tzBlock[1].replace(/[^\w_\-:,;=+/<br>]/g, '').split('<br>'); // collect timezone breakpoints (exactly 2)
  ...
  var actualDay = function () {
    if (tzBreakpoints["".concat(theCase)].day[0] == '-') {
      ...
      var dayIndex = Object.keys(weekdays["".concat(breakpointWeekday)]).length + 1 - parseInt(tzBreakpoints["".concat(theCase)].day[1]);
...
var atcbVersion = '1.17.0'; // CHECKING FOR SPECIFIC DEVICED AND SYSTEMS
// browser
...
var isAndroid = isBrowser() ? function () {
...
var isChrome = isBrowser() ? function () {
...
var isProblematicWebView = isBrowser() ? function () {
...
var atcbConsoleInit = false;
var atcbDefaultTarget = isWebView() ? '_system' : '_blank';
var atcbOptions = ['apple', 'google', 'ical', 'ms365', 'outlookcom', 'msteams', 'yahoo'];
var atcbValidRecurrOptions = ['apple', 'google', 'ical'];
var atcbiOSInvalidOptions = ['ical']; // DEFINING GLOBAL ICONS
...
function atcb_init() {
  ...
  var atcButtons = document.querySelectorAll('.atcb'); // if there are some, move on

  if (atcButtons.length > 0) {
    // get the amount of already initialized ones first
    var atcButtonsInitialized = document.querySelectorAll('.atcb-initialized'); // generate the buttons one by one
    ...
    var _loop = function _loop(i) {
      ...
      var atcbJsonInputPatched = atcb_patch_config(atcbJsonInput); // check, if all required data is available
      ...
      if (atcb_check_required(atcbJsonInputPatched)) {
        ...
        var atcbConfig = atcb_decorate_data(atcbJsonInputPatched); // set identifier
...
function atcb_patch_config(configData) {
  ...
  Object.keys(keyChanges).forEach(function (key) {
...
function atcb_decorate_data(data) {
  var now = new Date(); // format RRULE

  if (data.recurrence != null && data.recurrence != '') {
    // remove spaces and force upper case
    data.recurrence = data.recurrence.replace(/\s+/g, '').toUpperCase(); // pre-validate
    ...
    if (!/^(RRULE:[\w=;,:+-/\\]+|daily|weekly|monthly|yearly)$/im.test(data.recurrence)) {
      ...
      if (/^RRULE:/i.test(data.recurrence)) {
        ...
        var rruleParts = data.recurrence.substr(6).split(';');
        var rruleObj = new Object();
        ...
        data.recurrence_frequency = data.recurrence; // generate the RRULE from easy rules

        data.recurrence = 'RRULE:FREQ=' + data.recurrence + ';WKST=' + data.recurrence_weekstart + ';INTERVAL=' + data.recurrence_interval; // TODO: If "until" is given, translate it into a "count" and remove the "until" (here and in the above block). This would be way more stable!
  ...
  for (var i = 0; i < data.options.length; i++) {
    ...
    data.options["".concat(i)] = cleanOption[0].toLowerCase().replace('microsoft', 'ms').replace('.', '');

    if (cleanOption[1] != null) {
      data.optionLabels["".concat(i)] = cleanOption[1];
    } else {
      data.optionLabels["".concat(i)] = '';
    }
  ...
  for (var _i = 0; _i < data.options.length; _i++) {
    ...
    if (isiOS() && atcbiOSInvalidOptions.includes(data.options["".concat(_i)])) {
      data.options.splice(_i, 1);

      if (data.optionLabels["".concat(_i)] != null) {
        delete data.optionLabels["".concat(_i)];
      }
    ...
    if (data.recurrence != null && data.recurrence != '') {
      if (!atcbValidRecurrOptions.includes(data.options["".concat(_i)])) {
        data.options.splice(_i, 1);
      ...
      if (data.recurrence_until != null && data.recurrence_until != '' && (data.options["".concat(_i)] == 'apple' || data.options["".concat(_i)] == 'ical')) {
        data.options.splice(_i, 1);
  ...
  data = atcb_date_cleanup(data); // calculate the real date values in case that there are some special rules included (e.g. adding days dynamically)

  data.startDate = atcb_date_calculation(data.startDate);
  data.endDate = atcb_date_calculation(data.endDate); // set default listStyle
  ...
  if (data.description != null && data.description != '') {
    ...
    data.descriptionHtmlFree = atcb_rewrite_html_elements(data.description, true); // ...and transform pseudo elements for the regular one
...
function atcb_check_required(data) {
  ...
  var requiredField = ['name', 'startDate'];
...
function atcb_date_cleanup(data) {
  ...
  endpoints.forEach(function (point) {
    if (data[point + 'Date'] != null) {
      ...
      data[point + 'Date'] = data[point + 'Date'].replace(/\.\d{3}/, '').replace('Z', ''); // identify a possible time information within the date string

      var tmpSplitStartDate = data[point + 'Date'].split('T');
...
function atcb_date_calculation(dateString) {
  ...
  var today = new Date();
  var todayString = today.getUTCMonth() + 1 + '-' + today.getUTCDate() + '-' + today.getUTCFullYear();
  dateString = dateString.replace(/today/gi, todayString); // check for any dynamic additions and adjust
...
function atcb_validate(data) {
  ...
  var msgPrefix = 'Add to Calendar Button generation (' + data.identifier + ')'; // validate explicit ics file

  if (data.icsFile != null && data.icsFile != '') {
    if (!atcb_secure_url(data.icsFile, false) || !/\.ics$/.test(data.icsFile) || !data.icsFile.startsWith('https://')) {
  ...
  if (data.organizer != null && data.organizer != '') {
    var organizerParts = data.organizer.split('|');

    if (organizerParts.length != 2 || organizerParts[0].length > 50 || organizerParts[1].length > 80 || !atcb_validEmail(organizerParts[1])) {
      console.error(msgPrefix + ' failed: organizer needs to match the schema "NAME|EMAIL" with a valid email address');
      return false;
    }
  ...
  if (!/^(\w|-){1,254}$/.test(data.uid)) {
    console.error(msgPrefix + ': UID not valid. May only contain alpha, digits, and dashes; and be less than 255 characters');
    return false;
  ...
  if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-5][0-9a-f]{3}-[089ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(data.uid)) {
    console.warn(msgPrefix + ': UID is strictly recommended to be a hex-encoded random Universally Unique Identifier (UUID)!');
    data.sequence = 0;
  ...
  if (!/^\d{8}T\d{6}Z$/.test(data.created)) {
    console.error(msgPrefix + ': created date format not valid. Needs to be a full ISO-8601 UTC date and time string, formatted YYYYMMDDTHHMMSSZ');
    return false;
  }

  if (!/^\d{8}T\d{6}Z$/.test(data.updated)) {
    console.error(msgPrefix + ': updated date format not valid. Needs to be a full ISO-8601 UTC date and time string, formatted YYYYMMDDTHHMMSSZ');
    return false;
  } // validate status
  ...
  if (!dates.every(function (date) {
    if (data["".concat(date)].length !== 10) {
      console.error(msgPrefix + ' failed: date misspelled [-> YYYY-MM-DD]');
      return false;
    }

    var dateParts = data["".concat(date)].split('-');

    if (dateParts.length < 3 || dateParts.length > 3) {
      console.error(msgPrefix + ' failed: date misspelled [' + date + ': ' + data["".concat(date)] + ']');
      return false;
    }
  ...
  if (!times.every(function (time) {
    if (data["".concat(time)] != null) {
      if (data["".concat(time)].length !== 5) {
        console.error(msgPrefix + ' failed: time misspelled [-> HH:MM]');
        return false;
      }

      var timeParts = data["".concat(time)].split(':'); // validate the time parts

      if (timeParts.length < 2 || timeParts.length > 2) {
        console.error(msgPrefix + ' failed: time misspelled [' + time + ': ' + data["".concat(time)] + ']');
        return false;
      }

      if (timeParts[0] > 23) {
        console.error(msgPrefix + ' failed: time misspelled - hours number too high [' + time + ': ' + timeParts[0] + ']');
        return false;
      }

      if (timeParts[1] > 59) {
        console.error(msgPrefix + ' failed: time misspelled - minutes number too high [' + time + ': ' + timeParts[1] + ']');
        return false;
...
function atcb_generate_label(data, parent, type) {
  ...
  var defaultTriggerText = atcb_translate_hook('Add to Calendar', data.language, data); // if there is only 1 option, we use the trigger text on the option label. Therefore, forcing it here
  ...
  switch (type) {
    ...
    default:
      if (data.trigger === 'click') {
        ...
        parent.addEventListener('mouseenter', atcb_debounce_leading(function (event) {
          event.preventDefault();
    ...
    case 'close':
      ...
      parent.addEventListener('focus', atcb_debounce(function () {
        return atcb_close(false);
      }));
  ...
  if (!oneOption && type === 'trigger') {
    parent.addEventListener('keyup', atcb_debounce_leading(function (event) {
      if (event.key == 'Enter') {
        event.preventDefault();
...
function atcb_generate(button, data) {
  ...
  button.textContent = ''; // create schema.org data, if possible (https://schema.org/Event)
  ...
  if (data.name && data.location && data.startDate) {
    ...
    var formattedDate = atcb_generate_time(data, 'delimiters', 'general', true);
      ...
      if (data.recurrence_byDay != null && data.recurrence_byDay != '') {
        var byDayString = function () {
          if (/\d/.test(data.recurrence_byDay)) {
            ...
            var byDays = data.recurrence_byDay.split(',');
            var helperMap = {
    ...
    schemaContent.push(data.location.startsWith('http') ? '"eventAttendanceMode":"https://schema.org/OnlineEventAttendanceMode",\r\n"location": {\r\n"@type":"VirtualLocation",\r\n"url":"' + data.location + '"\r\n}' : '"location":"' + data.location + '"');
  ...
  button.appendChild(buttonTriggerWrapper); // generate the button trigger div
  ...
  buttonTriggerWrapper.appendChild(buttonTrigger); // generate the label incl. eventListeners
  ...
  if (data.options.length === 1) {
    ...
    atcb_generate_label(data, buttonTrigger, 'trigger', true, data.label); // create an empty anchor div to place the dropdown, while the position can be defined via CSS

    var buttonDropdownAnchor = document.createElement('div');
  ...
  button.classList.add('atcb-initialized'); // show the placeholder div
...
function atcb_generate_dropdown_list(data) {
  ...
  optionsList.style.fontSize = data.size + 'px'; // generate the list items
  ...
  data.options.forEach(function (option) {
    ...
    optionsList.appendChild(optionItem); // generate the label incl. individual eventListener
...
function atcb_generate_bg_overlay() {
  ...
  var lightMode = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 'light';
  ...
  bgOverlay.addEventListener('touchstart', atcb_debounce_leading(function () {
    return fingerMoved = false;
  }), {
    passive: true
  });
  bgOverlay.addEventListener('touchmove', atcb_debounce_leading(function () {
    return fingerMoved = true;
  }), {
    passive: true
  });
  bgOverlay.addEventListener('touchend', atcb_debounce(function (e) {
    if (fingerMoved !== false || e.target !== e.currentTarget) return;
    atcb_toggle('close');
  }), {
    passive: true
  });
  bgOverlay.addEventListener('focus', atcb_debounce_leading(function (e) {
    if (e.target !== e.currentTarget) return;
    atcb_toggle('close');
  }));
...
function atcb_open(data, button) {
  ...
  if (document.querySelector('.atcb-list') || document.querySelector('.atcb-modal')) return; // generate list and prepare wrapper

  var list = atcb_generate_dropdown_list(data);
  var listWrapper = document.createElement('div');
  listWrapper.classList.add('atcb-list-wrapper'); // set list styles, set button to atcb-active and force modal listStyle if no button is set
  ...
  var bgOverlay = atcb_generate_bg_overlay(data.listStyle, data.trigger, data.lightMode, data.background); // render the items depending on the liststyle

  var atcbL = document.createElement('div');
  ...
  setTimeout(function () {
    atcbL.innerHTML = '<a href="https://add-to-calendar-button.com" target="_blank" rel="noopener">' + atcbIcon['atcb'] + '</a>';
  }, 500);

  if (data.listStyle === 'modal') {
    document.body.appendChild(bgOverlay);
    bgOverlay.appendChild(list); //document.body.appendChild(atcbL);
    ...
    listWrapper.appendChild(list);
    listWrapper.classList.add('atcb-style-' + data.buttonStyle); //document.body.appendChild(atcbL);
  ...
  atcb_set_fullsize(bgOverlay); // give keyboard focus to first item in list, if not blocked, because there is definitely no keyboard trigger
...
function atcb_close() {
  ...
  Array.from(document.querySelectorAll('.atcb-active-modal')).forEach(function (button) {
    button.classList.remove('atcb-active-modal');
  }); // make body scrollable again

  document.body.classList.remove('atcb-modal-no-scroll'); // remove dropdowns, modals, and bg overlays (should only be one of each at max)

  Array.from(document.querySelectorAll('.atcb-list-wrapper')).concat(Array.from(document.querySelectorAll('.atcb-list'))).concat(Array.from(document.querySelectorAll('.atcb-info-modal'))).concat(Array.from(document.querySelectorAll('#add-to-calendar-button-reference'))).concat(Array.from(document.querySelectorAll('#atcb-bgoverlay'))).forEach(function (el) {
    return el.remove();
  });
...
function atcb_action(data, triggerElement) {
  ...
  data = atcb_secure_content(data); // decorate & validate data
  ...
  if (triggerElement) {
    data.identifier = triggerElement.id; // if listStyle some dropdown one, force overlay

    if (data.listStyle != 'modal') {
      data.listStyle = 'overlay';
    }
  } else {
    data.identifier = 'atcb-btn-custom'; // if no button is defined, fallback to listStyle "modal" and "click" trigger
...
function atcb_generate_google(data) {
  // url parts
  var urlParts = [];
  urlParts.push('https://calendar.google.com/calendar/render?action=TEMPLATE'); // generate and add date

  var formattedDate = atcb_generate_time(data, 'clean', 'google');
  urlParts.push('dates=' + encodeURIComponent(formattedDate.start) + '%2F' + encodeURIComponent(formattedDate.end)); // setting time zone if given and not GMT +/- something, since this is not supported by Google Calendar
  ...
  if (data.location != null && data.location != '') {
    urlParts.push('location=' + encodeURIComponent(data.location)); // TODO: Find a better solution for the next temporary workaround.
...
function atcb_generate_yahoo(data) {
  // url parts
  var urlParts = [];
  urlParts.push('https://calendar.yahoo.com/?v=60'); // generate and add date
...
function atcb_generate_microsoft(data) {
  ...
  urlParts.push(baseUrl); // generate and add date
...
function atcb_generate_teams(data) {
  // url parts
  var urlParts = [];
  var baseUrl = 'https://teams.microsoft.com/l/meeting/new?'; // generate and add date

  var formattedDate = atcb_generate_time(data, 'delimiters', 'microsoft');
  urlParts.push('startTime=' + encodeURIComponent(formattedDate.start));
  urlParts.push('endTime=' + encodeURIComponent(formattedDate.end)); // add details (if set)
...
function atcb_generate_ical(data) {
  ...
  var filename = 'event-to-save-in-my-calendar';
  ...
  var now = new Date();
  var formattedDate = atcb_generate_time(data, 'clean', 'ical');
  var ics_lines = ['BEGIN:VCALENDAR', 'VERSION:2.0'];
  ics_lines.push('PRODID:-// https://add-to-calendar-pro.com // button v' + atcbVersion + ' //EN');
  ics_lines.push('CALSCALE:GREGORIAN'); // we set CANCEL, whenever the status says so
  ...
  var timeAddon = function () {
    ...
    if (data.timeZone != null && data.timeZone != '') {
      var timeZoneBlock = tzlib_get_ical_block(data.timeZone);
  ...
  if (isiOS() && isChrome() || isWebView() && (isiOS() || isAndroid() && isProblematicWebView())) {
    ...
    tmpInput.remove(); // creating the modal

    if (isiOS() && isChrome()) {
      atcb_create_modal(data, 'browser', atcb_translate_hook('Crios iCal headline', data.language, data), atcb_translate_hook('Crios iCal info', data.language, data) + '<br>' + atcb_translate_hook('WebView iCal solution 1', data.language, data) + '<br>' + atcb_translate_hook('Crios iCal solution 2', data.language, data));
    } else {
      atcb_create_modal(data, 'browser', atcb_translate_hook('WebView iCal headline', data.language, data), atcb_translate_hook('WebView iCal info', data.language, data) + '<br>' + atcb_translate_hook('WebView iCal solution 1', data.language, data) + '<br>' + atcb_translate_hook('WebView iCal solution 2', data.language, data));
    }
  } else {
...
function atcb_save_file(file, filename) {
  try {
    ...
    save.rel = 'noopener';
    save.href = file; // not using default target here, since this needs to happen _self on iOS (abstracted to mobile in general) and _blank at Firefox (abstracted to other setups) due to potential cross-origin restrictions
...
function atcb_generate_time(data) {
  ...
  if (data.startTime != null && data.endTime != null) {
    ...
    if (data.timeZone != null && data.timeZone != '') {
      ...
      var offsetEnd = tzlib_get_offset(data.timeZone, data.endDate, data.endTime); // if we need to add the offset to the datetime string, do so respectively
    ...
    var _newEndDate = new Date(Date.UTC(endDate[0], endDate[1] - 1, endDate[2])); // increment the end day by 1 for Google Calendar, iCal and Outlook
...
function atcb_validEmail(email) {
  ...
  if (mx) {// TODO: call external service to validate email address
  }
...
function atcb_rewrite_html_elements(content) {
  ...
  if (clear) {
    content = content.replace(/\[(|\/)(url|br|hr|p|b|strong|u|i|em|li|ul|ol|h\d)\]|((\|.*)\[\/url\])/gi, ''); // and build html for the rest
...
function atcb_create_modal(data) {
  ...
  // setting the stage
  var bgOverlay = atcb_generate_bg_overlay('modal', 'click', data.lightMode);
  var infoModalWrapper = document.createElement('div');
  ...
  infoModalWrapper.appendChild(infoModal); // set overlay size just to be sure

  atcb_set_fullsize(bgOverlay); // adding closing button

  var infoModalClose = document.createElement('div');
  infoModalClose.classList.add('atcb-modal-close');
  infoModalClose.innerHTML = atcbIcon.close;
  infoModal.appendChild(infoModalClose);
  infoModalClose.addEventListener('click', atcb_debounce(function () {
    return atcb_close();
  }));
  infoModalClose.addEventListener('keyup', atcb_debounce_leading(function (event) {
    if (event.key == 'Enter') {
      event.preventDefault();
  ...
  var infoModalHeadline = document.createElement('div');
  infoModalHeadline.classList.add('atcb-modal-headline');
  infoModal.appendChild(infoModalHeadline);

  if (icon != '') {
    var infoModalHeadlineIcon = document.createElement('span');
    infoModalHeadlineIcon.classList.add('atcb-modal-headline-icon');
    infoModalHeadlineIcon.innerHTML = atcbIcon["".concat(icon)];
    infoModalHeadline.appendChild(infoModalHeadlineIcon);
  }

  var infoModalHeadlineText = document.createTextNode(headline);
  infoModalHeadline.appendChild(infoModalHeadlineText); // and text content
  ...
  infoModal.appendChild(infoModalContent); // and buttons (array of objects; attributes: href, type, label, primary(boolean))

  if (buttons != null && buttons.length > 0) {
    var infoModalButtons = document.createElement('div');
    ...
    buttons.forEach(function (button, index) {
      ...
      switch (button.type) {
        default:
        case 'close':
          infoModalButton.addEventListener('click', atcb_debounce(function () {
            return atcb_close();
          }));
          infoModalButton.addEventListener('keyup', atcb_debounce(function (event) {
            if (event.key == 'Enter') {
              atcb_toggle('close', '', '', true);
            }
          }));
...
function atcb_position_list(trigger, list) {
  ...
  if (anchorSet === true && !list.classList.contains('atcb-dropoverlay')) {
    ...
    var listWidth = triggerDim.width + 20 + 'px';
    list.style.minWidth = listWidth; // read list dimensions again, since we altered the width in the step before
...
if (isBrowser()) {
  ...
  document.addEventListener('keyup', atcb_debounce_leading(function (event) {
    if (event.key === 'Escape') {
      atcb_toggle('close', '', '', true);
    }
...
var i18nStrings = {
  en: {
    ...
    'WebView iCal info': 'Unfortunately, in-app browsers have problems with the way we generate the calendar file.',
    'WebView iCal solution 1': "We automatically put a magical URL into your phone's clipboard.",
    'WebView iCal solution 2': '<ol><li><strong>Open another browser</strong> on your phone, ...</li><li><strong>Paste</strong> the clipboard content and go.</li></ol>',
    'Crios iCal headline': 'Open Safari',
    'Crios iCal info': 'Unfortunately, Chrome on iOS has problems with the way we generate the calendar file.',
    'Crios iCal solution 2': '<ol><li><strong>Open Safari</strong>, ...</li><li><strong>Paste</strong> the clipboard content and go.</li></ol>'
  },
  de: {
    ...
    'WebView iCal info': 'Leider haben In-App-Browser Probleme mit der Art, wie wir Kalender-Dateien erzeugen.',
    'WebView iCal solution 1': 'Wir haben automatisch eine magische URL in die Zwischenablage deines Smartphones kopiert.',
    'WebView iCal solution 2': '<ol><li><strong>Öffne einen anderen Browser</strong> auf deinem Smartphone, ...</li><li>Nutze die <strong>Einfügen</strong>-Funktion, um fortzufahren.</li></ol>',
    ...
    'Crios iCal solution 2': '<ol><li><strong>Öffne Safari</strong>, ...</li><li>Nutze die <strong>Einfügen</strong>-Funktion, um fortzufahren.</li></ol>'
  },
  es: {
    ...
    'WebView iCal info': 'Lamentablemente, los browsers in-app tienen problemas con la forma en que generamos el archivo del calendario.',
    'WebView iCal solution 1': 'Hemos copiado automáticamente una URL mágica en el portapapeles de tu smartphone.',
    'WebView iCal solution 2': '<ol><li><strong>Abre otro browser</strong> en tu smartphone, ...</li><li>Utilice la función de <strong>pegar</strong> para continuar.</li></ol>',
    'Crios iCal headline': 'Abrir Safari',
    'Crios iCal info': 'Lamentablemente, Chrome en iOS tiene problemas con la forma de generar el archivo de calendario.',
    'Crios iCal solution 2': '<ol><li><strong>Abrir Safari</strong>, ...</li><li>Utilice la función de <strong>pegar</strong> para continuar.</li></ol>'
  },
  pt: {
    ...
    'WebView iCal info': 'Infelizmente, os navegadores em tampas têm problemas com a forma como geramos o ficheiro de calendário.',
    'WebView iCal solution 1': 'Copiámos automaticamente um URL mágico para a área de transferência do seu smartphone.',
    'WebView iCal solution 2': '<ol><li><strong>Abrir outro browser</strong> en tu smartphone, ...</li><li>Use a função <forte>colar</strong> para continuar.</li></ol>',
    'Crios iCal headline': 'Safari aberto',
    'Crios iCal info': 'Infelizmente, o cromado no iOS tem problemas com a forma como geramos o ficheiro do calendário.',
    'Crios iCal solution 2': '<ol><li><strong>Safari aberto</strong>, ...</li><li>Use a função <forte>colar</strong> para continuar.</li></ol>'
  },
  fr: {
    ...
    'WebView iCal info': 'Malheureusement, les navigateurs in-app ont des problèmes avec la manière dont nous créons les fichiers de calendrier.',
    'WebView iCal solution 1': 'Nous avons automatiquement copié une URL magique dans le presse-papiers de ton smartphone.',
    'WebView iCal solution 2': '<ol><li><strong>Ouvre un autre navigateur</strong> sur ton smartphone, ...</li><li>Utilise la fonction <strong>insérer</strong> pour continuer.</li></ol>',
    'Crios iCal headline': 'Ouvre Safari',
    'Crios iCal info': 'Malheureusement, Chrome sur iOS a des problèmes avec la façon dont nous générons le fichier du calendrier.',
    'Crios iCal solution 2': '<ol><li><strong>Ouvre Safari</strong>, ...</li><li>Utilise la fonction <strong>insérer</strong> pour continuer.</li></ol>'
  },
  nl: {
    ...
    'WebView iCal info': 'Helaas hebben in-app browsers problemen met de manier waarop wij kalenderbestanden maken.',
    'WebView iCal solution 1': 'We hebben automatisch een magische URL naar het klembord van uw smartphone gekopieerd.',
    'WebView iCal solution 2': '<ol><li><strong>Open een andere browser</strong> op uw smartphone, ...</li><li>Gebruik de <strong>insert</strong> functie om verder te gaan.</li></ol>',
    'Crios iCal headline': 'Open Safari',
    'Crios iCal info': 'Helaas heeft Chrome op iOS problemen met de manier waarop we het kalenderbestand genereren.',
    'Crios iCal solution 2': '<ol><li><strong>Open Safari</strong>, ...</li><li>Gebruik de <strong>insert</strong> functie om verder te gaan.</li></ol>'
  },
  tr: {
    ...
    'WebView iCal info': 'Ne yazık ki, uygulama içi tarayıcılar takvim dosyalarını oluşturma şeklimizle ilgili sorunlar yaşıyor.',
    'WebView iCal solution 1': 'Akıllı telefonunuzun panosuna otomatik olarak sihirli bir URL kopyaladık.',
    'WebView iCal solution 2': '<ol><li><strong>Akıllı telefonunuzda başka bir tarayıcı açın</strong>, ...</li><li>Devam etmek için <strong>insert</strong> fonksiyonunu kullanın.</li></ol>',
    'Crios iCal headline': 'Açık Safari',
    'Crios iCal info': "Ne yazık ki iOS'ta Chrome'un takvim dosyası oluşturma yöntemiyle ilgili sorunları var.",
    'Crios iCal solution 2': '<ol><li><strong>Açık Safari</strong>, ...</li><li>Devam etmek için <strong>insert</strong> fonksiyonunu kullanın.</li></ol>'
  },
  zh: {
    ...
    'WebView iCal solution 2': '<ol><li>打开手机上的任何其他浏览器, ...</li><li>粘贴剪贴板内容并开始.</li></ol>',
    'Crios iCal headline': '打开 Safari',
    'Crios iCal info': '不幸的是，iOS 上的 Chrome 在我们生成日历文件的方式上存在问题.',
    'Crios iCal solution 2': '<ol><li><strong>打开 Safari</strong>, ...</li><li>粘贴剪贴板内容并开始.</li></ol>'
  },
  ar: {
    ...
    'WebView iCal solution 2': '<ol><li>افتح أي متصفح آخر على هاتفك الذكي, ...</li><li>الصق محتوى الحافظة واذهب.</li></ol>',
    'Crios iCal headline': 'افتح Safari',
    'Crios iCal info': 'لسوء الحظ ، يواجه Chrome على iOS مشاكل في طريقة إنشاء ملف التقويم.',
    'Crios iCal solution 2': '<ol><li><strong>افتح Safari</strong>, ...</li><li>الصق محتوى الحافظة واذهب.</li></ol>'
  },
  hi: {
    ...
    'WebView iCal solution 2': '<ol><li>अपने फ़ोन पर <strong>दूसरा ब्राउज़र खोलें</strong>, ...</li><li>क्लिपबोर्ड सामग्री <strong>चिपकाएं</strong> और जाएं।</li></ol>',
    'Crios iCal headline': 'सफारी खोलें',
    'Crios iCal info': 'दुर्भाग्य से, iOS पर Chrome को कैलेंडर फ़ाइल जेनरेट करने के हमारे तरीके में समस्या है।',
    'Crios iCal solution 2': '<ol><li><strong>सफारी खोलें</strong>, ...</li><li>क्लिपबोर्ड सामग्री <strong>चिपकाएं</strong> और जाएं।</li></ol>'
  },
  pl: {
    ...
    'WebView iCal info': 'Niestety, przeglądarki in-app mają problemy ze sposobem, w jaki generujemy plik kalendarza.',
    'WebView iCal solution 1': 'Automatycznie umieszczamy magiczny adres URL w schowku telefonu.',
    'WebView iCal solution 2': '<ol><li><strong>Otwórz inną przeglądarkę</strong> w swoim telefonie, ...</li><li><strong>Wklej</strong> zawartość schowka i ruszaj.</li></ol>',
    ...
    'Crios iCal solution 2': '<ol><li><strong>Otwórz Safari</strong>, ...</li><li><strong>Wklej</strong> zawartość schowka i ruszaj.</li></ol>'
  },
  id: {
    ...
    'WebView iCal info': 'Sayangnya, browser dalam aplikasi memiliki masalah dengan cara kami menghasilkan file kalender.',
    'WebView iCal solution 1': 'Kami secara otomatis memasukkan URL ajaib ke clipboard ponsel Anda.',
    'WebView iCal solution 2': '<ol><li><strong>Buka peramban lain</strong> pada ponsel Anda, ...</li><li>Tempelkan konten clipboard dan pergi.</li></ol>',
    'Crios iCal headline': 'Buka Safari',
    'Crios iCal info': 'Sayangnya, Chrome di iOS memiliki masalah dengan cara kami menghasilkan file kalender.',
    'Crios iCal solution 2': '<ol><li><strong>Buka Safari</strong>, ...</li><li>Tempelkan konten clipboard dan pergi.</li></ol>'
  },
  no: {
    ...
    'WebView iCal info': 'Dessverre har nettlesere i appen problemer med måten vi genererer kalenderfilen på.',
    'WebView iCal solution 1': 'Vi legger automatisk inn en magisk URL i telefonens utklippstavle.',
    'WebView iCal solution 2': '<ol><li><strong>Åpne en annen nettleser</strong> på telefonen, ...</li><li><strong>Lim inn</strong> innholdet på utklippstavlen og gå.</li></ol>',
    ...
    'Crios iCal solution 2': '<ol><li><strong>Åpne Safari</strong>, ...</li><li><strong>Lim inn</strong> innholdet på utklippstavlen og gå.</li></ol>'
  },
  fi: {
    ...
    'WebView iCal info': 'Valitettavasti sovelluksen sisäisillä selaimilla on ongelmia kalenteritiedoston luomisessa.',
    'WebView iCal solution 1': 'Laitamme automaattisesti maagisen URL-osoitteen puhelimesi leikepöydälle.',
    'WebView iCal solution 2': '<ol><li><strong>Avaa toinen selain</strong> puhelimessasi., ...</li><li><strong>liitä</strong> leikepöydän sisältö ja lähde.</li></ol>',
    'Crios iCal headline': 'Avaa Safari',
    'Crios iCal info': 'Valitettavasti iOS:n Chromessa on ongelmia kalenteritiedoston luomisessa.',
    'Crios iCal solution 2': '<ol><li><strong>Avaa Safari</strong>, ...</li><li><strong>liitä</strong> leikepöydän sisältö ja lähde.</li></ol>'
  },
  sv: {
    ...
    'WebView iCal solution 2': '<ol><li><strong>Öppna en annan webbläsare</strong> på telefonen, ...</li><li><strong>Insätt</strong> innehållet i klippbordet och kör.</li></ol>',
    'Crios iCal headline': 'Öppna Safari',
    'Crios iCal info': 'Tyvärr har Chrome på iOS problem med hur vi genererar kalenderfilen.',
    'Crios iCal solution 2': '<ol><li><strong>Öppna Safari</strong>, ...</li><li><strong>Insätt</strong> innehållet i klippbordet och kör.</li></ol>'
  },
  cs: {
    ...
    'WebView iCal info': 'Prohlížeče v aplikacích mají bohužel problémy se způsobem generování souboru kalendáře.',
    'WebView iCal solution 1': 'Do schránky telefonu automaticky vložíme kouzelnou adresu URL.',
    'WebView iCal solution 2': '<ol><li><strong>Otevření jiného prohlížeče</strong> v telefonu, ...</li><li><strong>Vložte</strong> obsah schránky a přejděte.</li></ol>',
    ...
    'Crios iCal solution 2': '<ol><li><strong>Otevřít Safari</strong>, ...</li><li><strong>Vložte</strong> obsah schránky a přejděte.</li></ol>'
  },
  ja: {
    ...
    'WebView iCal solution 2': '<ol><li>スマートフォンで別のブラウザを起動する, ...</li><li>クリップボードの内容を貼り付けて行く。</li></ol>',
    'Crios iCal headline': 'オープンSafari',
    'Crios iCal info': '残念ながら、iOS版Chromeでは、カレンダーファイルの生成方法に問題があります。',
    'Crios iCal solution 2': '<ol><li><strong>オープンSafari</strong>, ...</li><li>クリップボードの内容を貼り付けて行く。</li></ol>'
  },
  it: {
    ...
    'WebView iCal info': 'Purtroppo i browser in-app hanno problemi con il modo in cui generiamo il file del calendario.',
    'WebView iCal solution 1': 'Inseriamo automaticamente un URL magico negli appunti del telefono.',
    'WebView iCal solution 2': '<ol><li><strong>Aprire un altro browser</strong> sul cellulare, ...</li><li><strong>Incollare</strong> il contenuto degli appunti e partire.</li></ol>',
    'Crios iCal headline': 'Aprire Safari',
    'Crios iCal info': 'Purtroppo, Chrome su iOS ha problemi con il modo in cui generiamo il file del calendario.',
    'Crios iCal solution 2': '<ol><li><strong>Aprire Safari</strong>, ...</li><li><strong>Incollare</strong> il contenuto degli appunti e partire.</li></ol>'
  },
  ko: {
    ...
    'WebView iCal solution 2': '<ol><li>휴대전화에서 다른 브라우저 열기, ...</li><li>클립보드 내용을 붙여넣고 이동합니다.</li></ol>',
    'Crios iCal headline': 'Safari 열기',
    'Crios iCal info': '불행히도 iOS의 Chrome은 캘린더 파일을 생성하는 방식에 문제가 있습니다.',
    'Crios iCal solution 2': '<ol><li><strong>Safari 열기</strong>, ...</li><li>클립보드 내용을 붙여넣고 이동합니다.</li></ol>'
  },
  vi: {
    ...
    'WebView iCal info': 'Rất tiếc, các trình duyệt trong ứng dụng gặp sự cố với cách chúng tôi tạo tệp lịch.',
    'WebView iCal solution 1': 'Chúng tôi tự động đặt một URL kỳ diệu vào khay nhớ tạm thời trên điện thoại của bạn.',
    'WebView iCal solution 2': '<ol><li><strong> Mở trình duyệt khác </strong> trên điện thoại của bạn, ...</li><li><strong> Dán </strong> nội dung khay nhớ tạm và bắt đầu.</li></ol>',
    'Crios iCal headline': 'Mở Safari',
    'Crios iCal info': 'Rất tiếc, Chrome trên iOS gặp sự cố với cách chúng tôi tạo tệp lịch.',
    'Crios iCal solution 2': '<ol><li><strong>Mở Safari</strong>, ...</li><li><strong> Dán </strong> nội dung khay nhớ tạm và bắt đầu.</li></ol>'
  }
