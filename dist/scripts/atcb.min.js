...
function tzlib_get_offset(tzName, isoDate, isoTime) {
  ...
  var timezoneData = tzBlock[1].replace(/[^\w_\-:,;=+/<br>]/g, '').split('<br>');
  var tzBreakpoints = {
    1: {},
    2: {}
  };
...
function atcb_decorate_data_dates(data) {
  if (data.dates != null && data.dates.length > 0) {
    ...
    data.endTime = data.dates[0].endTime = _cleanedUpDates.endTime;
    data.timeZone = data.dates[0].timeZone = _cleanedUpDates.timeZone;
...
function atcb_check_required(data) {
  ...
  if (data.dates != null && data.dates.length > 0) {
    ...
    var requiredSingleField = ['startDate'];
...
function atcb_validate_date_blocks(data, msgPrefix) {
  var _loop2 = function _loop2(i) {
    ...
    if (!atcb_validate_status(data, msgPrefix, i, msgSuffix)) return {
      v: false
    };
    if (!atcb_validate_availability(data, msgPrefix, i, msgSuffix)) return {
      v: false
    };
    if (!atcb_validate_organizer(data, msgPrefix, i, msgSuffix)) return {
      v: false
    };
    if (!atcb_validate_uid(data, msgPrefix, i, msgSuffix)) return {
      v: false
    };
    if (!atcb_validate_sequence(data, msgPrefix, i, msgSuffix)) return {
      v: false
    };
    if (!atcb_validate_timezone(data, msgPrefix, i, msgSuffix)) return {
      v: false
    };
    if (!atcb_validate_datetime(data, msgPrefix, i, msgSuffix)) return {
      v: false
    };
  };
    ...
    if (data.images != null) {
      if (Array.isArray(data.images)) {
        for (var _i5 = 0; _i5 < data.images.length; _i5++) {
          if (atcb_secure_url(data.images["".concat(_i5)]) && data.images["".concat(_i5)].startsWith('http')) {
            imageData.push('"' + data.images["".concat(_i5)] + '"');
          }
        }
      }
    } else {
...
function atcb_create_modal(data) {
  ...
  var modalCount = document.querySelectorAll('.atcb-modal').length;
  ...
  if (subEvents.length > 1) {
    ...
    var _loop3 = function _loop3(i) {
      ...
      switch (subEvents[0]) {
        ...
        case 'outlookcom':
        case 'yahoo':
          modalSubEventButton.addEventListener('click', atcb_debounce(function () {
...
function atcb_generate_date_button(data, parent) {
  ...
  var fullTimeInfo = function () {
    ...
    if (subEvent == 'all') {
      ...
      var formattedTime = atcb_generate_time(data.dates["".concat(subEvent)]);
  ...
  var btnDay = document.createElement('div');
  btnDay.classList.add('atcb-date-btn-day');
  btnLeft.appendChild(btnDay);
  var btnMonth = document.createElement('div');
  ...
  var btnDetails = document.createElement('div');
  btnDetails.classList.add('atcb-date-btn-details');
  btnRight.appendChild(btnDetails);
  var btnHeadline = document.createElement('div');
  ...
  if (data.location != null && data.location != '' || cancelledInfo == '') {
    ...
    if (cancelledInfo != '') {
      ...
      var btnLocationIcon = document.createElement('span');
      ...
      var btnLocationText = document.createElement('span');
  ...
  var btnDateTimeIcon = document.createElement('span');
